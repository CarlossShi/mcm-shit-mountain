...
###################################################################
    ...
    ############################################################
        ...
        if goto_load==1:##################上一秒刚刚进行了移动，立刻进入 上料 或 下料、上料、清洗状态
            goto_load=0
            for num in range(1, 9):
                if cnc[num - 1][t, 3] == 1 and num==mov_pro:  # 如果cnc是空闲,上有孰料状态
                    rgv[t, 2] = 3
                    judge_layoff[0, num - 1] = t + cnc_inf[num - 1][4] + w
                    load_num = load_num + 1
                    layoff_num = layoff_num + 1
                    ...
                    break  # 一个时间rgv只能进入一种状态，这里进入了下料、上料、清洗状态
                if cnc[num - 1][t, 3] == 0 and num==mov_pro:  # 如果cnc也是空闲，无料状态（初始状态）
                    rgv[t, 2] = 2  # rgv进入上料状态
                    judge_load[0, num - 1] = t + cnc_inf[num - 1][4] / 2  # 假设空cnc上料时间为上下料时间的一半
                    load_num = load_num + 1
                    print("第%d个CNC在第%d秒开始上第%d个料" % (num, t, load_num))
